...
function returnList($pst){
  ...
  if($pst == "returnlist"){
    ...
    $type = $_POST["type"];
    $sql = 'SELECT id,title,creator,artist,difficulty FROM scoreList';
    switch($type){
      case "none":
        ...
        break;
      case "word":
        $sql .= " WHERE title LIKE :word1 OR creator LIKE :word1 OR artist LIKE :word1 ORDER BY id desc";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(array(':word1' => "%$_POST[word1]%"));
        break;
      case "num":
        $sql .= ' WHERE difficulty BETWEEN ? AND ? ORDER BY difficulty desc';
        $num = intval($_POST['word1']);
        $stmt = $pdo->prepare($sql);
        $stmt->execute(array($num - 0.99, $num));
        break;
      case "words":
        $sql .= " WHERE (title LIKE :word1 OR creator LIKE :word1 OR artist LIKE :word1)";
      ...
      case "between":
        ...
        if($num1 > $num2){
          ...
          $sql .= ' WHERE difficulty BETWEEN ? AND ? ORDER BY difficulty asc';
        }
        $stmt = $pdo->prepare($sql);
