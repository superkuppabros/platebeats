...
function createScore(audio, saveD, infoD2, nowPage) {
  ...
  var pos = 0; //現在位置(0 - 47)
  var locate = function(x) {
    return 350 - (300 * x) / 192;
  };
  ...
  function changePage(pg) {
    ...
    if (page != 1) {
      if (
        page != maxPage &&
        infoData[page - 1][0] - infoData[page - 2][0] == 1
      ) {
        fixed = 1;
      } else {
        fixed = 0;
  ...
  function fixedSwitch() {
    if (page != 1) {
      if (fixed == 0) {
        ...
        var newNum =
          (28800 * term) / Number(infoData2[chapter - 1][1]) +
          Number(infoData2[chapter - 1][2]);
  ...
  button4.onclick = function() {
    var answer = confirm(
      "Current data is deleted if you return.\nAre you sure you want to return?"
    );
    if (answer) {
      cancelAllCreate();
    ...
    return function f(e) {
      ...
      if (code == 27) {
        ...
        if (chapter != 1) {
          ...
          var newNum =
            (28800 * term) / Number(infoData2[chapter - 2][1]) +
            Number(infoData2[chapter - 2][2]);
        ...
        l2.font = "20px 'Arial'";
        l2.fillText(
          "notes:" + difficulty[0] + " / ★:" + difficulty[1],
          175,
          32
        );
      } else if (code == 72) {
      ...
      return function f() {
        if (x > 10 && x < 170 && y > 335 && y < 395) {
          ...
          if (rank != "-" && ac >= 95) {
            ...
            layer2.removeEventListener("click", f, false);
            // submitData(audio, frameData, saveD, infoD2, scr, pg);
            testPlay(audio, frameDasta, saveD, infoD2, pg);
          }
        }
      };
    })(),
